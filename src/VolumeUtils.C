...
/// Helper function for writing VDB grids
    ...
    boost::shared_ptr<MeshPotato::MPUtils::Camera> buildFrustumCamera(MeshPotato::MPUtils::MPVec3 eye, openvdb::FloatGrid::Ptr grid) {
      ...
      maxdistance = std::max(maxdistance, (point1 - eye).dot(abboxview));
      maxAngle = std::max(maxAngle, 2*acos((point2 - eye).unit().dot(abboxview))*180/M_PI);
      mindistance = std::min(mindistance, (point2 - eye).dot(abboxview));
      maxdistance = std::max(maxdistance, (point2 - eye).dot(abboxview));
      maxAngle = std::max(maxAngle, 2*acos((point3 - eye).unit().dot(abboxview))*180/M_PI);
      mindistance = std::min(mindistance, (point3 - eye).dot(abboxview));
      maxdistance = std::max(maxdistance, (point3 - eye).dot(abboxview));
      maxAngle = std::max(maxAngle, 2*acos((point4 - eye).unit().dot(abboxview))*180/M_PI);
      mindistance = std::min(mindistance, (point4 - eye).dot(abboxview));
      maxdistance = std::max(maxdistance, (point4 - eye).dot(abboxview));
      maxAngle = std::max(maxAngle, 2*acos((point5 - eye).unit().dot(abboxview))*180/M_PI);
      mindistance = std::min(mindistance, (point5 - eye).dot(abboxview));
      maxdistance = std::max(maxdistance, (point5 - eye).dot(abboxview));
      maxAngle = std::max(maxAngle, 2*acos((point6 - eye).unit().dot(abboxview))*180/M_PI);
      mindistance = std::min(mindistance, (point6 - eye).dot(abboxview));
      maxdistance = std::max(maxdistance, (point6 - eye).dot(abboxview));
      maxAngle = std::max(maxAngle, 2*acos((point7 - eye).unit().dot(abboxview))*180/M_PI);
      mindistance = std::min(mindistance, (point7 - eye).dot(abboxview));
      maxdistance = std::max(maxdistance, (point7 - eye).dot(abboxview));
      maxAngle = std::max(maxAngle, 2*acos((point8 - eye).unit().dot(abboxview))*180/M_PI);
      mindistance = std::min(mindistance, (point8 - eye).dot(abboxview));
      maxdistance = std::max(maxdistance, (point8 - eye).dot(abboxview));
      maxAngle += 20;
      if (maxAngle > 90) {
        std::cout<< "WARNING: Light is too close and may not illuminate the entire density!" << std::endl;
      }
      maxAngle = std::min(89.9999, maxAngle);
      fnearP = mindistance;
